...
export const BookingDialog = ({ open, onOpenChange, property, initialCheckIn, initialCheckOut }: BookingDialogProps) => {
      ...
      navigate(`/booking-confirmation/${bookingNumber}`, {
        state: {
          booking: {
            ...
            base_amount: priceBreakdown?.baseAmount || 0,
            cleaning_fee: priceBreakdown?.cleaningFee || 0,
            tax_amount: priceBreakdown?.taxAmount || 0,
            status: 'confirmed',
            payment_status: 'paid',
  ...
  return (
    <Dialog
      open={open}
      onOpenChange={step === 'payment' ? undefined : onOpenChange}
      modal={step !== 'payment'}
    >
      <DialogPortal>
