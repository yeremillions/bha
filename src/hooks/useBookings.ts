...
export const checkAvailability = async ({
  ...
  try {
    // Optimization: Use RPC for standard availability checks (public facing)
    // This bypasses RLS issues for guest users who can't read the bookings table
    if (!excludeBookingId) {
      ...
      if (error) {
        console.error('Error in check_property_availability RPC:', error);
        // Fallback to query if RPC fails (e.g., if it doesn't exist yet)
      } else {
        return {
          available: data as boolean,
          // RPC doesn't return conflicting bookings, but we usually don't need them for basic checks
          conflictingBookings: [],
        };
      }
    }

    // Direct query fallback (for updates or if RPC fails/missing)
    // Requires 'bookings' table select permissions
    let query = supabase
      .from('bookings')
      .select('*')
...
export const useCreateBooking = (options?: CreateBookingOptions) => {
  ...
  return useMutation({
    mutationFn: async (newBooking: NewBooking) => {
      ...
      // Insert booking (booking_number will be auto-generated by database trigger)
      // Do NOT provide booking_number - let the trigger generate it (BK001, BK002, etc.)
      const { booking_number: _ignored, ...bookingWithoutNumber } = newBooking as any;

      const { data, error } = await supabase
        .from('bookings')
        .insert([
          {
            ...bookingWithoutNumber,
