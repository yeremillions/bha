-- =====================================================
-- BROOKLYN HILLS APARTMENTS - COMPLETE DATA RELOAD
-- This clears everything and reloads all data
-- =====================================================
...
BEGIN
  ...
  RAISE NOTICE '✅ Properties inserted: %', prop_count;
  IF prop_count != 4 THEN
    RAISE EXCEPTION 'Expected 4 properties but got %', prop_count;
  END IF;
END $$;
...
BEGIN
  ...
  RAISE NOTICE '✅ Customers inserted: %', cust_count;
  IF cust_count != 6 THEN
    RAISE EXCEPTION 'Expected 6 customers but got %', cust_count;
  END IF;
END $$;
...
BEGIN
  ...
  IF booking_count != 6 THEN
    RAISE EXCEPTION 'Expected 6 bookings but got %', booking_count;
  END IF;
END $$;
...
DECLARE
  trans_count INTEGER;
BEGIN
  SELECT COUNT(*) INTO trans_count FROM public.transactions;
  RAISE NOTICE '✅ Transactions inserted: %', trans_count;
  IF trans_count != 5 THEN
    RAISE EXCEPTION 'Expected 5 transactions but got %', trans_count;
  END IF;
END $$;
